...
class SlackStrategyBot extends BaseBot {
  ...
  _handleMessage(originalMessage) {
    ...
    this.strategyHandler.getAnswer(originalMessage).then(answer => {
      if (originalMessage.isGroup) {
        const group = this._getGroupById(originalMessage.channel);
        if (group) {
          this.logger.debug('writing to group', answer);
          this.postMessageToGroup(group.name, answer);
        }
      } else if (originalMessage.isChannel) {
        const channel = this._getChannelById(originalMessage.channel);
        if (channel) {
          this.logger.debug('writing to channel', answer);
          this.postMessageToChannel(channel.name, answer);
        }
      }
    });
  }
  ...
  _getChannelById(channelId) {
    return this._filterById(channelId, this.channels);
  }
